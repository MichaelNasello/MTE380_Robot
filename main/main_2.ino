...
class Robot{
  ...
  */
  ...
  /*
  The following variables are used to smoothen course corrections with PID control.
  */
  public:
  	float P, I, D, previous_error, PID_value = 0.0;
  ...
  while(true){
    ...
    if (robot.ir_blue_status and !rescued){
      ...
      if (counter == 0){
        ...
        // Turn left
        robot.set_motor_power(robot.motor_a_forwards, robot.motor_a_backwards, robot.motor_a_speed, counter - robot.PID_value, true);
        robot.set_motor_power(robot.motor_b_forwards, robot.motor_b_backwards, robot.motor_b_speed, counter + robot.PID_value, true);
        robot.set_motor_power(robot.motor_c_forwards, robot.motor_c_backwards, robot.motor_c_speed, counter - robot.PID_value, true);
        robot.set_motor_power(robot.motor_d_forwards, robot.motor_d_backwards, robot.motor_d_speed, counter + robot.PID_value, true);
      }
      else if (counter > 0){
      	// Turn right
        robot.set_motor_power(robot.motor_a_forwards, robot.motor_a_backwards, robot.motor_a_speed, counter + robot.PID_value, true);
